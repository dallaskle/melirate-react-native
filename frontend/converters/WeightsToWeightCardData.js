...
export const WeightsToWeightCardData = (weights) => {
    weights.sort((a, b) => {return b.weight_id - a.weight_id});
    let results = [
        {
            header: 'Body Weight',
            amount: 0, //76.1,
    ...
    results[1].amount = Math.round((weights[0].bodyFatPercent * 100)*decimals)/decimals 
    ...
    for (let i = 1; i < 4; i++) {
        ...
        results[1].changes[i - 1] = Math.round((weights[i].bodyFatPercent - weights[0].bodyFatPercent) * 100 *decimals)/decimals 
